...
const LoadableClassContent = Loadable({
...
export class ClassElement extends React.Component {
  ...
  render() {
    ...
    const { name, children, students, teacher, password, classes, classId, expanded, onChange, onDelete } = this.props;
    return (
      <ExpansionPanel expanded={expanded} onChange={onChange}>
        <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
          <Typography className={classes.heading}>{name}</Typography>
          <Typography className={classes.secondaryHeading}>{`Teacher: ${capitalizeName(teacher)}`}</Typography>
        </ExpansionPanelSummary>
        <ExpansionPanelDetails>
          <LoadableClassContent studentCount={students.length} classId={classId} />
        </ExpansionPanelDetails>
      </ExpansionPanel>
    );
  }
